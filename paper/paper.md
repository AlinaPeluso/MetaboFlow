...
This workflow proposes tools for processing Ionomics data to aid reproducible data sharing and big data initiatives. It consists of four main analysis:
...
We also perform the Go terms enrichment of the genes in the clusters by employing the GO terms annotation in the SGD online database `[@engel2014reference]`.
...
The workflow is available within the `IonFlow` R package which can be imported as
...
We illustrate the Ionomics workflow with ICP-MS data of yeast intracellular ion concentrations measured for 1454 single-gene haploid knockouts (data from `@danku2009high`). Ions measured include Ca44, Cd111, Co59, Cu65, Fe56, K39, Mg25, Mn55, Mo95, Na23, Ni60, P31, S34, Zn66. Values of concentration are in ppm and have been adjusted using optical density measurements. Intracellular concentrations are measured for two, four or eight replicas of each mutant depending on the knock-out. Knock-out YDL227C mutant is measured multiple times in every batch as control strain.
...
After the removal of the oulier values and the median batch correction of the logged concentrations, the data looks as

```
pre_proc$plot.logConcentration_by_batch 
```

![plot.logConcentration_by_batch](./Figures/pre_proc/plot.logConcentration_by_batch.png)
...
#### Symbolization
As we are working with the logConcentration (corrected and normalised) we can consider a thresold based on a certain number of $\sigma$ (e.g. thresold = 3$\sigma$) to symbolizise the concentrations' profile of the knockouts as follow:

* Symb=0  if logConcentration in (-3,3)  
* Symb=1  if logConcentration greater or equal than 3
* Symb=-1 otherwise
...
```
![plot.logConcentration_z_scores](./Figures/pre_proc/plot.logConcentration_z_scores.png)
...
```
![plot.Pearson_correlation](./Figures/exp_anal/plot.Pearson_correlation.png)
...
```
![plot.PCA_Individual](./Figures/exp_anal/plot.PCA_Individual.png)
...
```
![plot.heatmap](./Figures/exp_anal/plot.heatmap.png)
...
```
![plot.pairwise_correlation_map](./Figures/exp_anal/plot.pairwise_correlation_map.png)
...
```
![plot.regularized_partial_correlation_network](./Figures/exp_anal/plot.regularized_partial_correlation_network.png)
...
```
![plot.profiles](./Figures/gene_clust/plot.profiles.png)
...
gene_net$plot.pnet
```
![plot.pnet](./Figures/gene_net/plot.pnet.png)
...
```
![plot.impact_betweenees](./Figures/gene_net/plot.impact_betweenees.png)
